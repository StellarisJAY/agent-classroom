...
- `image` 元素在阶段一由模型产出 `prompt`（此时 `src` 为空），随后由**图片生成环节**调用文生图模型生成图片、写入对象存储并回填 `src`；多张图片并发生成（上限 4 并发，IO 密集型 API 调用），按原元素顺序回填；生成失败的元素直接剔除，不阻断流程。
...
- **图片生成**：阶段一产出的 `image` 元素（含 `prompt`）在进入阶段二前完成图片生成与 `src` 回填（多图并发，上限 4），故阶段二看到的元素清单已含可用图片。
